...
export async function startBot(userId: number, username: string, password?: string, sessionData?: any): Promise<{ success: boolean; message: string }> {
  try {
    ...
    // The error is likely coming from the loginInstagram function.
    // The original error message "رسالة غير معروفة" (Unknown message) suggests that the error object is not being correctly converted to a string.
    // We will ensure that the error message is always a string.
    const errorMessage = error.message || 'An unknown error occurred during bot start. Please check your credentials and session file.';
    // For tRPC to handle the error gracefully, we should throw a new error with a string message
    // or ensure the returned object is correctly formatted. Since we are returning an object,
    // the issue is likely that the client-side is not expecting a non-string error message.
    // We will ensure the message is a string and return the object.
